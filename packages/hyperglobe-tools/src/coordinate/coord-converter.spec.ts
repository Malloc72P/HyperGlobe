...
describe('CoordinateConverter', () => {
  describe('convert', () => {
    ...
    it('경도 90도, 위도 0도 → [0, 0, -1]', () => {
      const result = CoordinateConverter.convert([90, 0]);
    ...
    it('경도 0도, 위도 90도 (북극점) → [0, 1, 0]', () => {
      const result = CoordinateConverter.convert([0, 90]);
      expect(result[0]).toBeCloseTo(0, 5);
      expect(result[1]).toBeCloseTo(1, 5);
      expect(result[2]).toBeCloseTo(0, 5);
    });

    it('서울 좌표 (126.978, 37.566) 변환', () => {
      const seoul: Coordinate = [126.978, 37.566];
      const result = CoordinateConverter.convert(seoul);

      console.log('서울 convert 결과:', result);
  ...
  describe('invert', () => {
    ...
    it('서울 좌표 왕복 변환 테스트 (convert → invert)', () => {
      ...
      // 원본과 비교
      expect(inverted[0]).toBeCloseTo(original[0], 5); // 경도
      expect(inverted[1]).toBeCloseTo(original[1], 5); // 위도
    });
    ...
    it('여러 도시 좌표 왕복 변환 테스트', () => {
      ...
      cities.forEach((original, index) => {
        ...
        if (Math.abs(original[1]) < 89) {
          expect(inverted[0]).toBeCloseTo(original[0], 4); // 경도
        }
        expect(inverted[1]).toBeCloseTo(original[1], 4); // 위도
      });
    });
  });
});
