...
// 결과: faceGeometry, sideGeometry, borderPoints, useVertexColors
// (mesh가 아니라 BufferGeometry/좌표 배열을 반환)
...
hyperglobe -i ./world-low.geo.json -o ./world-low.hgm
```
...
interface HGMFeature {
  ...
  // 이미 삼각분할 완료된 데이터
  geometries: GeometrySource[];  // vertices, indices 포함
  borderLines: BorderlineSource; // 외곽선 좌표
  bbox: BoundingBox;             // 공간 인덱싱용
}
```
...
interface RegionModel<DATA_TYPE = any> extends BoundingBox {
  id: string;              // 지역 고유 ID
  name: string;            // 지역 이름
  // minX/minY/maxX/maxY는 BoundingBox에서 상속됨
  bboxSize: number;        // BBox 면적 (작은 지역 우선)
  polygons: FeaturePolygons[]; // Point-in-Polygon 검사용 (Coordinate[][]와 동일)
  data?: DATA_TYPE;        // 선택적 데이터
  properties: Record<string, any>;
}
```
...
interface HGMFeature {
  ...
  // 메쉬 (이미 삼각분할 완료)
  geometries: GeometrySource[];  // 각 항목이 vertices: Float32Array, indices: Uint32Array

  // 외곽선 (이미 구체에 투영됨)
  borderLines: BorderlineSource; // pointArrays: Float32Array[]
...
# GeoJSON → HGM 변환
hyperglobe -i ./my-map.geo.json -o ./output/my-map.hgm
```
