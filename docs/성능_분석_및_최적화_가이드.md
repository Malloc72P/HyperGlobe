...
- **측정 지표**: FPS (초당 프레임 수), 드로우콜 수, 지오메트리 수
...
### Priority 1: 빌드 타임 전처리 및 HGM 포맷 ⭐⭐⭐⭐⭐

**기대 효과:**
- 초기 로딩: 2-7초 → 0.3초 (약 25배 빠름)
- 파일 크기: 1.16MB → 0.8MB (30% 감소, gzip)
- CPU/GPU 부하 최소화
- 사용자 경험 극대화
...
|--------|------|---------|-----------|----------|
| **기본** | 58-60 fps, 1-2초 로딩 | - | - | ✅ 완료 |
| **포인터 이벤트** | 비활성화 | 60 fps | hover 복원 | ⭐⭐⭐⭐⭐ 필수 |
| **HGM 포맷** | 1-2초 로딩 | 0.3초 로딩 | 25배 빠름 | ⭐⭐⭐⭐ 권장 |
...
현재 성능이 목표(58-60fps)를 달성했으나, **포인터 이벤트가 비활성화**되어 있음
...
#### **우선순위 2: HGM 포맷 (빌드 타임 전처리)** ⭐⭐⭐⭐ (권장)
   - 목적: 초기 로딩 25배 개선 (2-7초 → 0.3초)
   - 방법: 이진 파일 포맷 + 삼각분할/좌표변환 전처리
   - 파일 크기: 1.16MB → 0.8MB (30% 감소)
   - 맵차트 기능 영향: 없음 (API 개선)
   - 구현 난이도: 높음 (1-2주)
   - 추가 효과: 
...
*문서 최종 수정일: 2025년 11월 6일*
*최적화 완료: RegionFeature 병합 (v1.0)*
*테스트 환경: M1 MacBook, world-high.geo.json*
