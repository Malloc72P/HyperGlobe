...
# 2. 결과물 생성
# world-high.geo.json → world-high.triangulated.json
...
```
현재:                       58-60 fps
+ 포인터 이벤트 최적화:        60 fps (hover 기능 복원)
+ 빌드타임 삼각분할:          60 fps (초기 로딩 개선)
```
...
#### **우선순위 2: 빌드 타임 삼각분할** ⭐⭐⭐⭐ (권장)
   - 목적: 초기 로딩 개선 (2-3초 → 0.5초 이하)
   - 우선순위: 높음
   - 맵차트 기능 영향: 없음
   - 구현 난이도: 중간
