...
파일 크기: 261KB (GeoJSON) → 714KB (HGM, gzip 압축)
...
#### world-low 기준 (HGM 포맷 적용)
| 지표 | 최적화 전 (GeoJSON) | HGM 포맷 적용 후 | 목표 |
|------|-------------------|----------------|------|
| **파일 크기** | 261KB | 714KB (gzip) | 작을수록 좋음 |
| **초기 로딩** | 3-7초 | < 0.5초 ✅ | < 1초 |
| **FPS** | 60 | 60 ✅ | 60 |
| **브라우저 연산** | 삼각분할 3-7초 | 거의 없음 ✅ | 최소화 |
| **병목 함수** | triangulatePolygon | graticule (0.23ms) ✅ | 없음 |
...
# 성능 검증
- [x] world-low.hgm 생성 및 테스트
- [x] 초기 로딩 시간 측정 (< 0.5초)
- [x] Chrome DevTools 퍼포먼스 분석 (병목 없음)
```

**Phase 3: 포인터 이벤트 (경위도 방식)** 🔜 진행 예정
```bash
# 1. 의존성 설치
npm install rbush @turf/turf @turf/boolean-point-in-polygon
npm install -D @types/rbush
...
*최적화 완료: RegionFeature 병합 (v1.0), HGM 포맷 (v2.0)*
*테스트 환경: M1 MacBook, world-low.hgm*
