...
**방안 A: 간소화된 충돌 감지용 Geometry**

**방안 A: 간소화된 충돌 감지용 Geometry**

병합으로 인한 레이캐스트 부담을 해결하는 방법
...
**단점:**
- ⚠️ 메모리 사용량 약간 증가 (~20%)
- ⚠️ 여전히 256개 지오메트리 검사 필요 (Globe 방식보다 느림)

**미채택 이유:**
- Globe 벡터 방식이 더 우수한 성능 제공 (99.9% vs 90%)
- Globe 방식이 더 간결한 아키텍처 (관심사 분리)
...
**미채택 이유:**
- Globe 벡터 방식에 이미 Throttle이 포함됨 (50ms)
- 단독으로는 성능 문제 해결 불가
...
**단점:**
- ⚠️ 3D → 2D 투영 동기화 복잡
- ⚠️ 지구본 회전 시 매 프레임 재계산 필요
- ⚠️ 구현 난이도 매우 높음

**미채택 이유:**
- 복잡도 대비 성능 이득 미미 (Globe 벡터 방식이 이미 충분히 빠름)
- 3D/2D 동기화로 인한 버그 가능성

---

**방안 D: Spatial Index (R-tree) - RegionFeature 기반**

레이캐스트 전 후보군 필터링

**핵심 아이디어:**
- R-tree로 마우스 근처 지역만 사전 필터링
- 필터링된 후보만 레이캐스트 수행
...
- ⚠️ 구현 복잡
- ⚠️ 회전/줌 시 좌표 변환 필요
- ⚠️ 여전히 3D 레이캐스트 수행 필요

**미채택 이유:**
- Globe 벡터 방식이 R-tree를 이미 활용하면서도 3D 레이캐스트를 회피
- 복잡도 대비 성능 이득 부족

---

**방안 E: Globe 벡터 히트 테스트 (최종 채택) ⭐⭐⭐⭐⭐**

위의 "4. 포인터 이벤트 레이캐스트 비용" 섹션에 상세 구현 참조.

**선택 이유:**
1. **최고의 성능**: 99.9% 부하 감소 (< 1ms per frame)
2. **간결한 구조**: 관심사 분리 (Globe: 상호작용, RegionFeature: 렌더링)
3. **확장 가능성**: 중앙 집중식 이벤트 처리
4. **유지보수성**: R-tree 단일 진입점
5. **구현 용이성**: 기존 코드 최소 변경

---

#### **성능 비교 요약**

| 방안 | 레이캐스트 | 추가 연산 | 총 시간 | 구현 난이도 | 선택 |
|------|-----------|----------|---------|------------|------|
| A (간소화 메쉬) | 256개 × 200 삼각형 | - | ~2-3ms | 낮음 | ❌ |
| B (Throttle) | 256개 × 1,979 삼각형 | - | ~10ms | 매우 낮음 | ❌ |
| C (HTML Overlay) | 없음 | 3D→2D 투영 매프레임 | ~5ms | 매우 높음 | ❌ |
| D (R-tree + 레이캐스트) | 3개 × 1,979 삼각형 | R-tree | ~2ms | 중간 | ❌ |
| **E (Globe 벡터)** | **1개 구체** | **벡터변환 + R-tree + PiP** | **< 1ms** | **중간** | **✅** |

---

#### **결론**

Globe 벡터 히트 테스트 방식이 다음 이유로 최종 채택되었습니다:

1. **압도적 성능**: 다른 모든 방안보다 10배 이상 빠름
2. **아키텍처 우수성**: 렌더링과 상호작용의 명확한 분리
3. **실용성**: 구현 난이도 대비 최고의 효과
4. **확장성**: 향후 다른 인터랙션 추가 용이
...
**Priority 1.5: HGM API 개선** (진행 중)
- `<HyperGlobe mapData={hgm} />` 한 줄로 로드
- RegionFeature 자동 생성
- 로딩 상태 UI

**Priority 1.6: 맵 팩 제공** (예정)
- `@hyperglobe/maps` 패키지
- world-mid.hgm, world-high.hgm 제작
- 다양한 지역 맵

**Priority 2: 전체 메쉬 병합** (불필요)
- 현재 성능 충분
- 동적 기능 보존이 더 중요
